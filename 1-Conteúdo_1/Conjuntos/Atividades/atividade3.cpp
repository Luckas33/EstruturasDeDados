...
public:
    ...
    bool cheio();
    bool vazio();
    bool pertence(TipoItem item, int i, int f);
...
bool Conjunto::pertence(TipoItem item, int i, int f)
{
    int m = (i + f) / 2;
    ...
    if (item < estrutura[m]){
     return pertence(item,i, (m - 1));
    }
    else
    {
        return pertence(item,m + 1,f);
    }
    return false;
}
